...
class SkinvendAPI
{
    ...
    constructor(apiKey, secretKey, options = {})
    {
      ...
      this.client = axios.create({
        ...
        paramsSerializer: params => qs.stringify(params, { arrayFormat: 'brackets',encode: true, skipNulls: true })
      });
      ...
      this.client.interceptors.request.use((config) =>
      {
        ...
        if (config.params && Object.keys(config.params).length > 0) dataToSign = config.params;
        else if (config.data && Object.keys(config.data).length > 0) dataToSign = config.data;

        if (dataToSign)
        {
          try {config.headers['X-Signature'] = this.generateSignature(dataToSign, config.headers['X-Timestamp'])} 
          catch (error) { return Promise.reject(new Error('Failed to generate request signature'))}
        }
    ...
    CreateDeposit = async (deposit_id, steam_id = null, trade_url = '', min_amount = 0.5) =>
    {
        ...
        const {data} = await this.client.post('deposit', 
        {
          deposit_id,
          steam_id,
          trade_url,
    ...
    CreateOffer = async (deposit_id, steam_id = null, app_id = this.priorityGame, trade_url = '', item_array = []) =>
    {
        ...
        const {data} = await this.client.post('deposit', { deposit_id, steam_id });
        const {result} = await this.client.post('deposit', { app_id, trade_id: data.trade_id, trade_url, item_array });
        return result.data;
    }
    /**
    ...
    GetClientSteamInvetory = async (steam_id, refresh = false, app_id = this.priorityGame) =>
    {
      if(!steam_id)
        ...
        const {data} = await this.client.get('inventory', { params:{ app_id, steam_id, refresh } });
        return data;  
    }
    /**
     * Search Skins.
